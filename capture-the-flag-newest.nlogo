...
to move
  ...
  if any? allies   [
    ...
    ask allies with [any? enemy-flags in-radius 1] [
  ...
  ask enemy-flags with [any? ally-flags in-radius 1 and not [carried?] of one-of ally-flags] [
    ask links [die]
    move-to patch (min-pxcor + 3 ) (min-pycor + 5 )
    set carried? false
    set carrier nobody
  ...
  ask enemies [
    ifelse any? patches in-cone 10 60 with [ wall? = true ] [
      rotate-left
    ]
    [ fwd ]
  ]
...
to handle-collisions
  ...
  if any? ally-bullets [
    ask ally-bullets with [[wall?] of patch-ahead 0] [die]
    ask ally-bullets with [any? enemies in-radius 1][die]
  ]
